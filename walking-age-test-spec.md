...
**バージョン:** v7.0
**最終更新:** 2026-03-22
**対象ファイル:** `index.html` / `netlify/functions/proxy.js` / `netlify/functions/shoe-info.js`（shoe-info.js は index.html から直接呼び出しておらず独立 API）
...
10. [デプロイメント環境](#10-デプロイメント環境)
    - 10.3 [GAS 連携（別途管理）](#103-gas-連携別途管理)
...
| PDF レポート | 最大 5 ページ A4（患者向け・詳細アドバイス・地域在住者向け・臨床向け・靴推薦、表示設定により可変） |
...
| PWA | manifest.json + Service Worker | オプション |
...
| `height` | number | — | 身長（cm、レポート表示 + IP推定の脚長算出に使用） |
...
3 軸加速度 (x, y, z)
  ...
  → CV, SI, cadence, RMS, mean step time
      ...
      ▼
  速度・歩幅の自動推定
  → 速度: 定常区間推定距離 ÷ 定常区間時間差（Mode B; fallback: 設定距離 ÷ 全ピーク時間差）
  → 歩幅: speed ÷ (cadence / 60)
```
...
| セクション | 内容 |
|-----------|------|
| Section 1: 転倒・生活機能リスク判定 | 信号機カード3枚（転倒リスク・地域活動 / 余命・健康寿命 / 歩幅） |
| Section 2: フレイル・サルコペニアスクリーニング | AWGS 2019基準（速度・ケイデンス・歩行年齢スコア） |
| Section 3: 歩行の質 | CV・SI・歩行速度（3枚カード） |
| Section 4: 推奨介入アドバイス | 速度ティア（A/B/C）別エビデンスベース介入提案（CV ≥ 5.0 % でリズム訓練追加アラート表示。注: ADVICE_DB の自動選択では CV > 4.0% で `rhythmImpaired` 判定を行う。Page 2 の地域在住向けアラートはより保守的な 5.0% 閾値を使用する意図的な設計） |
...
| `auto` かつ速度 ≤ 0 | `unknown` | 速度データ未入力。スコアリング時は `low`（ACT_IDX フォールバック）として処理 |
...
> **速度フィルタ設定根拠:** ルール1の速度フィルタは 0.9 m/s（安全マージンとして 1.0 m/s より低く設定）。膝悩みの有無に関わらず適用。
...
| 3. 歩行速度 | 速度帯別に stability/cushion/rocker/lightweight をマッチ（< 0.8 m/s は fasten=any 時のみ easyOn +8 を加算） | 20–28 |
| 4. バイオメカニクス | CV/SI 閾値超過時に stability/archSupport をマッチ | 10 |
| 5. 設問ニーズ | Q1膝(23) + Q2防水(20) + Q3足(15–30) + Q4着脱(15) + Q6デザイン(15) | 73–103 |
...
www.asics.com
shop.asics.com
# プライベートIPレンジ（phyphox WiFi用）
192.168.x.x            # クラスC LAN（同一Wi-Fiネットワーク上のスマートフォン）
10.x.x.x               # クラスA LAN
172.16–31.x.x          # クラスB LAN
```

> **注:** プライベートIPはNetlify本番環境からは到達不可能なため、SSRF脅威にはならない。`IS_NETLIFY=true` 環境では phyphox WiFi 機能自体が利用不可（ローカルのみ対応）。

**レスポンス:**
- HTML モード: `text/plain` でページ HTML またはデータをそのまま返却
- 画像モード: `text/plain` で base64 データ URL を返却（`data:<mime>;base64,...`）
- phyphox モード: phyphox が返す JSON を `text/plain` でそのまま中継（`Content-Type: text/plain`）。クライアント側 `fetchPhyphoxRemote()` は `Response.json()` で受信するため Content-Type に依存しない。
...
4. HTML 全文（最大 100 KB）から靴特徴キーワードを抽出（35 ルール）
...
**特徴キーワード抽出ルール（35 件）:** fuzeGEL, GEL, FlyteFoam, Boost, React, Nike Air, クッション, 衝撃吸収, 安定性, ガイドレール, モーションコントロール, グルーヴ設計, トラスティック, アーチサポート, インソール, OrthoLite, 4E幅, 3E幅, 2E幅, 幅広, ロッカー, 屈曲性, 推進補助, 前傾設計, 軽量, GORE-TEX, 防水, 通気, 反発, サイドジッパー, ベルクロ, BOAフィット, シニア向け, ウォーキング, トレイル
...
`manifest.json` と Service Worker（`sw.js`）を配置することでインストール可能な PWA として動作する。

| ファイル | 役割 |
|---------|------|
| `manifest.json` | アプリ名・アイコン・テーマカラーなどのメタ情報 |
| `sw.js` | オフラインキャッシュ（静的アセット）・バックグラウンドフェッチ |

**環境別動作:**
- Netlify / http:// — PWA インストール可能（HTTPS 要件を満たす場合）
- GAS — Service Worker は GAS 環境では登録不可のため、`IS_GAS=true` 時は登録をスキップする

---

## 11. 規準値テーブル（NORM）

出典: 複数文献による合成規範値（Bohannon & Andrews Physiotherapy 2011 N=23,111/41研究; Hollman et al. Gait Posture 2011 N=294; Mobbs et al. Sensors 2025 N=313; Schwesig et al. Gait Posture 2011 N=1,860）
...
`assessInterventionProfile()` で歩行プロファイルから条件フラグを生成。Walk Ratio の `inefficientGait` 判定には 0.38–0.42 m/Hz の閾値を使用する（臨床向けページの正常範囲 0.35–0.45 とは別の、アドバイス早期介入用の厳密閾値）。
`selectAdvice()` で最大5件を自動選択（通常は4件程度。速度Bかつリズム異常など複数フラグが立った場合に5件に達することがある）。左パネルのチェックリストで手動調整可能。
